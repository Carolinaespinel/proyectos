$(function()
{
	...
	var letraMuestra = function()
	{
		...
		var animations =['bounce','shake',' hinge', 'pulse','tade','jello',];
		var rand = animations[Math.floor(Math.random() * animations.length)];
	
		$(".letra_" + letraPone).addClass("animated "+rand);
	};
	...
	$(document).keypress(function(event)
	{
		...
		if(event.keyCode >= 97 && event.keyCode <= 122 && inicia)
		{
			...
			//console.log("Veces letra presionada:", numeroVeces);
			console.log("teclapresionada"+numVeces);
			//animaciones de salida
		var a =["bounceOutUp","bounceOutDown","bounceOutRight","bounceOutLeft"];
		...
		{
			puntaje=puntaje+10;
			nom_div("puntaje").innerHTML="puntaje" + puntaje;
